...
export class EphemeralConversationMemory {
    ...
    constructor(options) {
        ...
        this.maxTextCharacters = positiveInteger(options.maxTextCharacters ?? 2_000, "maxTextCharacters");
    ...
    remember(actorId, userText, assistantText) {
        const id = opaqueActorId(actorId);
        const user = boundedText(userText, this.maxTextCharacters, "userText", "invalid_conversation");
        const assistant = boundedText(assistantText, this.maxTextCharacters, "assistantText", "invalid_conversation");
        const now = currentTime(this.now);
        this.removeExpired(now);
        ...
        if (!Number.isSafeInteger(expiresAt)) {
            throw new ConversationPolicyError("invalid_conversation", "The conversation expiry exceeds the safe time range.");
        }
    ...
    evictOldestActor() {
        while (this.conversations.size > this.maxActors) {
            const oldestActor = this.conversations.keys().next().value;
...
export function inferConversationPreferences(turns, options = {}) {
    const casualTurnThreshold = positiveInteger(options.casualTurnThreshold ?? 3, "casualTurnThreshold");
...
export function parseEvidenceSummary(input) {
    const summary = strictObject(input, ["schema", "answer", "confidence", "evidence", "caveats"], "evidence summary");
    ...
    const evidence = summary.evidence.map((entry, index) => {
        const item = strictObject(entry, ["statement", "source"], `evidence entry ${index}`);
        return {
            statement: boundedText(item.statement, 500, `evidence[${index}].statement`, "invalid_evidence_summary"),
            source: boundedText(item.source, 300, `evidence[${index}].source`, "invalid_evidence_summary"),
        };
    });
    const caveats = summary.caveats.map((caveat, index) => boundedText(caveat, 400, `caveats[${index}]`, "invalid_evidence_summary"));
    return {
        schema: "gossip.evidence-summary.v1",
        answer: boundedText(summary.answer, 1_000, "answer", "invalid_evidence_summary"),
        confidence,
        evidence,
        caveats,
    };
}
export function formatEvidenceSummary(input, options = {}) {
    const summary = parseEvidenceSummary(input);
    const language = options.language ?? "en";
    const voice = options.voice ?? "neutral";
    const maxEvidenceEntries = positiveInteger(options.maxEvidenceEntries ?? summary.evidence.length, "maxEvidenceEntries");
    ...
    const caveats = summary.caveats
        .map((caveat) => `• ${normalizeCompactReply(caveat, { maxCharacters: 220 })}`)
        .join("\n");
    ...
    return truncateText(segments.map(completeSentence).join("\n"), options.maxCharacters ?? 2_000);
}
export function normalizeCompactReply(input, options = {}) {
    if (typeof input !== "string") {
        throw new ConversationPolicyError("invalid_conversation", "Reply must be a string.");
    }
    const maximum = positiveInteger(options.maxCharacters ?? 2_000, "maxCharacters");
...
function hasCasualSignal(text) {
    return (/\p{Extended_Pictographic}/u.test(text) ||
        /(?:!{2,}|\?{2,})/u.test(text) ||
        /\b(?:ben|bro|coucou|genre|haha|hey|lol|mec|ngl|ouais|salut|stp|yo)\b/iu.test(text) ||
        /\b(?:j['’]ai|j['’]suis|t['’]as|tu|ça|i['’]m|you['’]re|can['’]t|don['’]t|let['’]s)\b/iu.test(text));
}
...
function summaryLabels(language, voice) {
    if (language === "fr") {
        return {
...
function positiveInteger(value, name) {
    if (!Number.isSafeInteger(value) || value <= 0) {
        throw new ConversationPolicyError("invalid_conversation", `${name} must be a positive safe integer.`);
    }
    return value;
}
function currentTime(now) {
    const value = now();
    if (!Number.isSafeInteger(value) || value < 0) {
        throw new ConversationPolicyError("invalid_conversation", "The conversation clock must return a non-negative safe integer.");
    }
    return value;
}
function opaqueActorId(value) {
    return boundedText(value, 256, "actorId", "invalid_conversation");
}
function boundedText(value, maximum, name, code) {
...
function invalidSummary(message) {
