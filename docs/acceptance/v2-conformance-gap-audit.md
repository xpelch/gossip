...
clean Sherwood checkout and consumes one exact TRX result containing the eleven
...
| Public submission, correction, exact retrieval and scoped sessions                                   | Conditional           | The eleventh process test proves signed HTTP/MCP submission, byte-identical receipts, restart durability, correction lineage, missing-target rollback, root-only lookup and zero-cost session scope. It leaves `public_submission` installed pending production acceptance. |
